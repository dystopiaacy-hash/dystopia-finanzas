-- =====================================================================
-- 017_metricas_ventas_fix_cash.sql
-- Reemplaza las vistas de la 016. Unico cambio: como se calcula el cash.
--
-- BUG DE LA 016: sumaba cc_dia1 + cc_cerrado + cc_seguimiento, y esas
-- columnas se solapan. Verificado el 2026-09-22 contra fin_pagos:
-- liam agosto daba 60.930 en Data contra 35.855 en Pagos, casi el doble.
--
-- Que es cada columna, segun los datos:
--   cc_cerrado     = VALOR TOTAL del trato. En liam, 92 filas cumplen
--                    cc_cerrado >= cc_dia1 + cc_seguimiento.
--   cc_dia1        = parte pagada el primer dia. En mauro, 157 de 235
--                    filas tienen cc_dia1 = cc_cerrado: pagaron todo junto.
--   cc_seguimiento = lo pagado despues.
--
-- Entonces: cobrado = cc_dia1 + cc_seguimiento.
--
-- IGUAL: el cash que factura sale SIEMPRE de fin_pagos, como fija
-- CONTRATO.md. Lo de Data es lo declarado en el CRM y sirve para
-- atribuir la venta a un closer, no para facturar. Por eso la columna
-- se llama cc_cobrado_data y no cc_mes.
--
...
select
  ...
  -- Cobrado: lo que efectivamente entro segun el CRM.
  coalesce(l.cc_dia1, 0)
    + coalesce(l.cc_seguimiento, 0)            as cc_cobrado,
  -- Valor del trato: cuanto vale la venta, este cobrada o no.
  coalesce(l.cc_cerrado, 0)                    as cc_valor_trato,
    ...
    case
      when nullif(btrim(l.estado_llamada), '') is not null
        then upper(btrim(l.estado_llamada)) in
             ...
             + coalesce(l.cc_seguimiento, 0) > 0  -- cualquier senal de plata
    end
...
select
  ...
  round(sum(cc_cobrado)     filter (where es_agendada), 2) as cc_cobrado_data,
  round(sum(cc_valor_trato) filter (where es_cerrada), 2)  as valor_tratos_cerrados,
  ...
  round(sum(cc_valor_trato) filter (where es_cerrada)
...
order by periodo desc, cliente_id, cc_cobrado_data desc nulls last;
...
with (security_invoker = true) as
with d as (
  select cliente_id, periodo, sum(cc_cobrado) as cc_data
  from fin_v_llamadas_clasificadas
  where es_agendada
  group by cliente_id, periodo
),
...
select cliente_id, periodo, agendadas, presentadas, show_up_rate,
       cerradas, tasa_cierre_total, tasa_cierre_calificadas,
       cc_cobrado_data, valor_tratos_cerrados, llamadas_sin_estado
from fin_v_metricas_ventas
where periodo >= to_char(current_date - interval '3 months', 'YYYY-MM')
order by periodo desc, cliente_id;

-- Conciliacion: con el cash corregido, la diferencia contra fin_pagos
-- tiene que achicarse mucho en los meses ya cargados (julio y agosto).
select * from fin_v_conciliacion_cc
where periodo between '2026-07' and '2026-08'
order by periodo desc, cliente_id;
